...
def pytest_collection_modifyitems(config, items):
    ...
    ``PLIMSOL_RUN_QUARANTINED=1`` runs them anyway — the way to WORK on a
    quarantined suite (round 4) — and says so on every such test, so a green
    run under the flag is never mistaken for the gate."""
    run_anyway = os.environ.get("PLIMSOL_RUN_QUARANTINED") == "1"
    for item in items:
        for key, (finding, owner, why) in QUARANTINE.items():
            if key in item.nodeid:
                if run_anyway:
                    item.add_marker(pytest.mark.quarantined_but_run(finding))
                else:
...
def pytest_configure(config):
    config.addinivalue_line(
        "markers", "quarantined_but_run(finding): a quarantined test run on purpose "
        "under PLIMSOL_RUN_QUARANTINED=1 — not the gate")
