...
def _seed_object_with_vr_at_new_version(object_name: str) -> int:
    ...
    with get_tenant_connection(TEST_TENANT_ID) as conn:
        ...
        obj = conn.execute(text(ins), {"et": "Object", "api": object_name, "vf": v}).scalar()
        vr = conn.execute(text(ins),
                          {"et": "ValidationRule", "api": f"{object_name}.Rule", "vf": v}).scalar()
        conn.execute(text(
...
def test_enqueue_pins_text_env_and_current_version(seeded):
    ...
    job = enqueue_s3_generation(
        tenant_id=TEST_TENANT_ID, requirement_ref=ref, environment_id=42, created_by=5)

    seq, name = resolve_current_s1_version(TEST_TENANT_ID)
...
def test_first_full_vertical_enqueue_to_complete(seeded):
    ...
    job = enqueue_s3_generation(
        tenant_id=TEST_TENANT_ID, requirement_ref=ref, environment_id=7, created_by=1)
