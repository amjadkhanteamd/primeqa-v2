...
def _seed_full(session) -> None:
    ...
    persist_interpretation(session, _interp(
        recipe_id=rid, claim_test_id=ct, outcome="passed",
        verdict="asserted_metadata_present"))
    persist_interpretation(session, _interp(
        recipe_id=rid, claim_test_id=ct, outcome="failed",
        verdict="asserted_metadata_absent"))
...
def test_assemble_empty_when_unseeded(session):
    ...
    assert out["interpretations"] == [] and out["grounding"] == []
    assert out["cause_clusters"] == [] and out["vr_clusters"] == []
    assert out["flapping"] == []


def test_interp_dict_shape(session):
    persist_interpretation(session, _interp(
        recipe_id=uuid4(), claim_test_id=uuid4(), outcome="failed",
        verdict="prohibition_not_enforced",
        cause_kind="enforcement_gap", vr_name="VR_B"))
    session.flush()
    row = _assemble_insights(session, limit=50)["interpretations"][0]
    assert row["outcome"] == "failed"
    assert row["verdict"] == "prohibition_not_enforced"
    assert row["cause"] == {"cause_kind": "enforcement_gap",
                            "vr_name": "VR_B", "detail": None}
    assert row["has_phrasing"] is False
