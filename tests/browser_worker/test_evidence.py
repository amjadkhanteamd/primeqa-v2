...
def test_build_keys_follow_the_scheme():
    from primeqa.browser_worker.evidence import build_keys
    k = build_keys("tenant_7", "m1", "j1", "home", 2)
...
def s3_and_prefix():
    ...
    for key in ev.list_keys(s3, bucket, prefix):
...
def test_round_trip_upload_verify_sign(s3_and_prefix):
    ...
    keys = ev.build_keys(prefix, "m", "j", "s", 1)
    obs = {"status": "OK", "n": 1}
    rec = ev.put_evidence(s3, bucket, keys, PNG_1x1, obs)
    ...
    assert ok, detail
    url = ev.sign_url(s3, bucket, keys["screenshot"], expires_s=300)
...
def test_checksum_mismatch_is_not_verified(s3_and_prefix):
    ...
    s3, bucket, prefix = s3_and_prefix
    keys = ev.build_keys(prefix, "m", "j", "s", 1)
    rec = ev.put_evidence(s3, bucket, keys, PNG_1x1, {"status": "OK"})
...
def test_missing_object_is_not_verified(s3_and_prefix):
    from primeqa.browser_worker import evidence as ev
    s3, bucket, prefix = s3_and_prefix
    rec = ev.EvidenceRecord(keys=ev.build_keys(prefix, "m", "j", "gone", 1),
...
def test_signed_url_expires(s3_and_prefix):
    ...
    s3, bucket, prefix = s3_and_prefix
    keys = ev.build_keys(prefix, "m", "j", "s", 1)
    ev.put_evidence(s3, bucket, keys, PNG_1x1, {"status": "OK"})
    url = ev.sign_url(s3, bucket, keys["screenshot"], expires_s=1)
    time.sleep(3)
