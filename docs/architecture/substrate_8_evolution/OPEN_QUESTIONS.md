...
- **S8-Q-007 — Re-interpretation temporality / contemporaneous grounding.** Grounding reads the *current* S1 by default. Should the run-time VR state be snapshotted (into evidence or a manifest) so a much-later evaluation reads the *contemporaneous* org rather than today's? (Cross-ref S6 `DEFERRED_ITEMS` item 1.)
