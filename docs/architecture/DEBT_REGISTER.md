...
| D6 | **Capture-block duplication across the two walk sites** (Wave 3 CP10 finding) — premise/create-effect/update-effect/subflow-call capture code exists in both the pre-decision stepper and `_walk_linear`. PARTIALLY REDUCED by the Completion Program (D-371): the two bounded FILTER parsers unified into one grammar (`_fb_parse_bounded_filters` — the inline twin had already drifted a NotEqualTo apart, proving the risk real); the loop grammar (aggregate + collection-update) lives at ONE site only (the stepper — post-decision loops remain a named limitation, not a duplicate). The remaining duplication: premise/create/update/subflow capture. A full walker unification is NOT an obviously-safe extraction (the two sites carry different conservatism context) — schedule as its own reviewed slice. | One walk, one capture surface. | none (latent drift risk) | M | Medium — the drifted-parser instance is fixed; unify the walkers in a dedicated slice. |
| D5 | ~~**Walk-site asymmetry**~~ — RESOLVED at Wave 3 CP2 (ee807d0): the stepper walks through subject-safe elements with identical conservatism semantics. The structural duplication became D6. **Walk-site asymmetry** (Wave 2 CP8 finding) — the pre-decision stepper stops at the first out-of-grammar element (pre-C5 semantics) while `_walk_linear` walks THROUGH subject-safe elements; premise capture is duplicated at both sites. Unifying would let post-lookup elements parse (no grounding change expected — verify by corpus diff) and collapse the duplication. | One walk, one conservatism surface. | FL05/FL12 paths | S–M | Medium — fold into C7 (whose evidence work touches exactly these paths). |
...
| E2 | ~~**Witness-synthesis module boundary**~~ — CLOSED at C3: `primeqa/generation/witnesses.py` is the entry point; the transform witness moved there and the band-interval generator (the fourth class) was born there. `verified_negative` / `decision_branch` generators stay in place, referenced from the module docstring. | Discoverability + reuse of typed-value discipline. | FL03 | — | Closed. |
