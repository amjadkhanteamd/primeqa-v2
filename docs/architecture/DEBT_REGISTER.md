...
| D6 | **Capture-block duplication across the two walk sites** (Wave 3 CP10 finding) — premise/create-effect/update-effect/subflow-call capture code exists in both the pre-decision stepper and `_walk_linear` (the D5 SEMANTIC unification exposed the structural duplication). Unify into one walker. | One walk, one capture surface. | none (latent drift risk) | M | High — fold into CP3's evidence slice (it touches exactly these paths). |
