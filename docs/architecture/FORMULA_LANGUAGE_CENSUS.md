...
| `ISCHANGED(f)` update-context | ok | pair compare; absence either side → NE; no ctx → NE | sound; *(superseded 2026-08-11, D-448)* the D-441 blanket >1-mutation guard is replaced by the **ordered-fold prior** (create ⊕ preceding SUCCEEDED same-record mutations, evidence order; rejected intermediates contribute nothing) — VR05's real 3-step evidence now evaluates True via the production chain where naive pairing gave the wrong-direction False; the guard **survives as refusal** for unknown intermediate outcomes, a deleted subject, and no/multiple base creates | **A** |
