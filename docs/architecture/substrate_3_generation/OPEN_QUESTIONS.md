...
- **Drift event judgment framework formalization.** Theme 6 (D-090 c) ships v1 with drift events annotated with `regression` | `evolution` | `neutral` by substrate-3 maintainers. Formalized judgment criteria (per-archetype, per-category) may emerge from Theme 7 quality envelope work as longitudinal data accumulates and patterns become discernible.
