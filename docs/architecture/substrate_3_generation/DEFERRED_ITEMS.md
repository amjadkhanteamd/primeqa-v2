...
- **`value-claim` positives — production-reachable mechanism LANDED (D-115.3, Option Q resolved); LLM-guidance + eval deferred.** The full S3 path is built: `_author_positive` (side A) + the **governance grounding stash** — `resolve_intent` now grounds a *field-and-value-specific* `GroundedPositive` (verify-at-grounding: the **named** field must `BELONGS_TO` the object; the value rides the intent's `target_subject_hint.expected_value` verbatim) — + `EMITTABLE += value-claim` (the gate-flip, landed in lockstep with the stash + the drift-guard). A real grounded value-claim now `PROCEED`s to emit; field-but-no-value → `EMISSION_DEFERRED` (no fabrication, D-115 §2); unknown field → `insufficient_grounding`. **Still deferred (the live reach):** the propose tool-description / prompt guidance telling the LLM to supply `field_name` + `expected_value`, and a **live eval probe** confirming a real requirement emits end-to-end (mirrors the negative — mechanism first, eval probe separate). — **D-100.3 / D-115 / D-115.1 / D-115.3**
