...
- **Approval-arc entry-criteria arming.** The D-337 staged-state guard closes the VR half of the req-302 authoring-waste class; its INVERSE (an arc whose staged conditions fail to ARM the bound approval process — the live `NO_APPLICABLE_PROCESS` catch) needs the arc grounding to check the approval's entry criteria are satisfiable from the staged conditions. Blocked on S1: ApprovalProcess capture carries no entry criteria today (no `entryCriteria` anywhere in the model); when S1 captures them, the check is the same Kleene walk `vr_conflict` already ships, pointed at the entry formula with the REQUIRE-ARM posture (the D-307.1 absence precedent), not the refuse-on-provable-fire one. — **D-337 / D-333 / D-320**
