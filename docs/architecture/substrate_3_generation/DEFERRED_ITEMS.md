...
- **Configuration `existence-claim` / `property-claim` — LANDED (D-122, Phase 2 slice 1).** Both emittable + grounded (Layer-1-complete, no caveat): existence via `get_entities`, property via `get_entity_details` (invent-nothing — the value is read from S1, a mismatch refuses). The S1 detail-read increment they were tied to shipped in Phase 0 (D-119). **Residual:** property's grounded *integration* test (a seeded `field_details` row) + the prompt live-reach (the LLM proposing them, slice 1b). — **D-098.4 / D-100.3 / D-122**
- **Permission `capability-claim` — LANDED (D-123, Phase 2 slice 2).** Direct profile / permission-set grants (read / edit on object / field) emittable + grounded (Layer-1-complete, no caveat): `_resolve_permission` verifies the `GRANTS_OBJECT_ACCESS` / `GRANTS_FIELD_ACCESS` edge's specific `can_read` / `can_edit` **bit** via `get_related` — edge-present-with-bit-set IS the verification; bit-unset or no edge refuses. v1 grounds direct grants only; sharing / OWD / role-hierarchy capability stays deferred (§6, D-080). **Residual:** the prompt live-reach (the LLM proposing it, slice 4 / D-125). — **D-100.4 / D-080 / D-123**
- **UI `layout-claim` — LANDED (D-124, Phase 2 slice 3).** "Field F appears on PageLayout L" emittable + grounded (Layer-1-complete, no caveat): `_resolve_ui` verifies the `INCLUDES_FIELD` (Layout→Field) edge via `get_related` — placement-existence IS the verification (no capability-style bit; edge-existence is the placement); absent → refuse. A **metadata-recipe, NOT a ui-recipe** — placement is a metadata fact (the runtime render/enable surface is `element-state-claim`, §6, Tier-3). v1 grounds placement-existence; section/row/column assertions are a v1.1 refinement. **Residual:** the prompt live-reach (slice 4 / D-125). — **D-100.4 / D-081 / D-124**
