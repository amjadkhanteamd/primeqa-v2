...
- **`value-claim` positives — emission-authored (D-115 side A); production-reachability held.** The author-capability landed: `GroundedPositive` + `_author_positive` (a value-claim + a Create→Read→Assert-equals data recipe); `author_emission` dispatches it. **Two pieces are held *together*** so a real grounded value-claim still defers gracefully (`emission-deferred`): (1) `EMITTABLE += value-claim` — it drives `resolve_intent`'s proceed-gate, so it must land with the stash (Option Q, D-115.1); (2) the **governance grounding stash** — `resolve_intent` does not yet build a *field-and-value-specific* `GroundedPositive` (today's value-claim grounding is object-level — confirms the object *has* a field; the intent descriptor carries no specific field or value), which needs the **synthesis→intent `{field, expected_value}` contract** (the next S3-grounding decision, after the S4 execution spine). — **D-100.3 / D-115 / D-115.1**
