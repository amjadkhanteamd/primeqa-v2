...
## 2026-06-02 — Permission breadth: capability-claim emission (Phase 2 slice 2, D-123)

The second Phase-2 (S3 breadth) slice and the **permission-archetype debut** — the emittable surface grows **5 → 6**. `capability-claim` asserts "**Profile / PermissionSet P grants read / edit on Object / Field X**" — the one permission kind that is **S1-Tier-1-groundable today** (the `GRANTS_OBJECT_ACCESS` / `GRANTS_FIELD_ACCESS` edges are synced — ~11K FieldPermissions on the sandbox — and `get_related` returns their `can_read` / `can_edit` properties, proven Phase 0 / D-120). **Layer-1-complete, no caveat** — reading the configured grant IS the verification (D-079).

**Shape — two shipped patterns fused.** capability-claim combines the **two-endpoint** grounding of `metadata-relationship-claim` (a grantee + a target + the grant edge between them) with the **metadata-inspection** recipe of the configuration kinds. New S2 body `CapabilityClaimBody` (`granting_subject` / `target` IdentityBearingRef, `granted_capability`, `grant_type: object|field`) — additive (`CLAIM_KIND_ENUM` already held it, D-121; deserializes via the flat `ClaimBody` union, **no migration**). `GroundedCapability` + `_author_capability` reuse `_inspection_recipe` (read the grantee, capture the grant edge). `EMITTABLE += (permission, capability-claim)`, kept lockstep with `_EMITTABLE_SHAPES`.

**Grounding — checks the BIT, not mere edge existence.** `_resolve_permission` (dispatch on `archetype_hint == "permission"`): resolve the grantee + target, then verify the grant edge via `get_related` **and** that its specific capability bit is set (`edit` → `can_edit`, mapped through `_CAPABILITY_FLAG`). Edge absent OR bit unset → ungrounded refusal — a claim asserting *edit* does NOT ground on an edge that only grants *read* (D-080: the recipe-kind preserves claim semantics, never overstates verification). `check_refs_exist` gains a two-endpoint (grantee + target) permission branch, mirroring metadata-relationship's source / target. **v1 grounds direct grants only**; sharing / OWD / role-hierarchy capability (S1 Tier-2, unmodeled) has no direct grant edge and refuses — the honest D-080 posture, not a defect. The run-as-execution upgrade reopens at S1 Phase-3.

**Verified.** 1076 generation + representation tests green (3 emit-probes + the `_EMITTABLE_SHAPES` drift-guard authoring the kind + two integration grounding tests on real seeded S1: grounds + emits, and **bit-unset refuses** — the test that proves the bit-check is real). Also repaired the `test_substrate_imports` registry drift-guard, red since D-122 (existence / property never enumerated — the D-122 green run covered the `generation/` suites but not this representation guard). **Follow-on:** the prompt live-reach (the LLM proposing capability-claim) batches into slice 4 / D-125. No S1 change, no migration. DECISIONS_LOG D-123. On `phase-10-substrate-3-breadth`.

## 2026-06-03 — UI breadth: layout-claim emission (Phase 2 slice 3, D-124)

The third Phase-2 (S3 breadth) slice and the **UI-archetype debut** — the emittable surface grows **6 → 7**, **completing the S1-Tier-1-groundable set** (the remaining 9 kinds are all Tier-2/Tier-3/integration-entity-blocked → S1 Phase-3). `layout-claim` asserts "**Field F appears on PageLayout L**".

**Verify-data-first gate → BUILD (the discipline the slice required).** Unlike `state-transition-claim` (dormant: the sync author's docstring records "this sandbox has **zero** record-triggered flows → zero `TRIGGERS_ON` edges", phases.py — built-correct-but-grounds-nothing, deferred Phase 0), the layout grounding edge is **live**: `phase_layout` is a fully-implemented, active sync phase whose docstring documents **~115 layouts → ~3,000–12,000 `INCLUDES_FIELD` edges** on the dev sandbox (phases.py:846/881). `INCLUDES_FIELD` (Layout→Field, grid-placement properties) is in the locked `TIER_1_EDGES`, and `get_related` traverses it generically (no new S1 work). The one residual — *live*-confirming instances — is the same standard capability-claim (D-123) was built on; every Salesforce org universally ships page layouts, so the dormancy risk is near-zero.

**Shape — a clone of capability-claim (D-123), `archetype="ui"`, `edge="INCLUDES_FIELD"`.** Two endpoints (Layout + Field) + a **metadata-inspection** recipe. **Layer-1-complete, no caveat** (D-079). New S2 body `LayoutClaimBody` (`layout` / `field` IdentityBearingRef) — additive (`layout-claim` already in `CLAIM_KIND_ENUM`, `"ui"` in `ARCHETYPE_ENUM`, D-121; deserializes via the flat `ClaimBody` union, **no migration**). `GroundedLayout` + `_author_layout` reuse `_inspection_recipe` (read the Layout, capture the `INCLUDES_FIELD` edge). `EMITTABLE += ("ui","layout-claim")`, kept lockstep with `_EMITTABLE_SHAPES`. `_resolve_ui` (dispatch on `archetype_hint == "ui"`) resolves the Layout + Field and verifies the `INCLUDES_FIELD` edge via `get_related`; `check_refs_exist` gains a two-endpoint ui branch.

**Key design call — metadata-recipe, NOT ui-recipe.** Layout placement is a *metadata* fact (the layout includes the field), verified by inspection — `metadata-recipe` / `inspection-trigger`, like capability/config. The `ui-recipe` / `ui-trigger` kinds stay reserved for `element-state-claim` (the *runtime* render/enable question — S1 Tier-3, deferred). A UI-interaction recipe for a placement claim would overstate verification (the D-080 honesty, applied to the UI archetype). v1 grounds **placement-existence** (no capability-style bit — edge-existence IS the placement); section/row/column assertions are a v1.1 refinement.

**Verified.** 1081 generation + representation tests green (3 emit-probes — including the metadata-not-ui assertion — + the `_EMITTABLE_SHAPES` drift-guard authoring the kind + two integration grounding tests on real seeded S1: grounds + emits, and **field-absent refuses** + the `test_substrate_imports` guard). **Follow-on:** the prompt live-reach (the LLM proposing layout-claim) batches into slice 4 / D-125. No S1 change, no migration. DECISIONS_LOG D-124. On `phase-10-substrate-3-breadth`.

## 2026-06-03 — Tier-1 breadth live-reach: prompt generation@v3 + eval corpus (Phase 2 slice 4, D-125)

The fourth Phase-2 slice — the **end-to-end live-reach** for the four kinds slices 1–3 made emittable + groundable (existence / property D-122, capability D-123, layout D-124). They ground + emit when *handed* an intent, but the LLM had no guidance to *propose* them — unreachable from a real requirement. This slice closes that, batched into one prompt freeze (the value-claim live-reach precedent, D-115.4).

**The de-risking finding — the tool schema is already permissive.** `propose_semantic_intent`'s `_ARCHETYPES` / `_CLAIM_KINDS` already enumerate `permission`/`ui` + `capability`/`layout`/`existence`/`property` (verbatim from S2's enums), and `target_subject_hint` is a free-form `{"type":"object"}` (no fixed key set). So **zero tool-schema / Layer-A change** — the live-reach is purely prompt fragments + the freeze + eval probes.

**Prompt fragments + the freeze (`generation@v2` → `v3`).** Each fragment names the *exact* `target_subject_hint` keys the resolver reads (the precision bar `data_behavior.md` set): `configuration.md` adds existence (flat `{entity_type, sf_api_name}`) + property (+`property_name`/`expected_value`); `permission.md` is sharpened to the four-key capability hint (`grantee`/`target`/`granted_capability`/`grant_type`) with the bit-check note; a new `ui.md` teaches the layout `{layout, field}` hint (metadata, not runtime). `_FRAGMENTS += "ui"`; `compose_working()` → freeze `versions/generation_v3.md`; record its SHA-256 in `RECORDED_HASHES`; `CURRENT = "generation@v3"`. v1/v2 stay frozen + pinned-resolvable (replay determinism, D-103.1); the hash-guard loops `versions()` so v3 is auto-covered.

**Eval corpus + the D-104 revisit.** +3 offline+live probes (`config-existence-draft` / `permission-capability-draft` / `ui-layout-draft`) — each scripts a propose+emit against a fixture and asserts a Layer-1 no-caveat draft with the right archetype / claim_kind / metadata-recipe, plus a live envelope (authored-but-skipped, periodic). One runner one-liner: `eval/runner.py:_seed` reads edge `properties` from the fixture (mirrors the entity-`attributes` read) so the capability probe's grant edge carries `can_edit:true` — additive, every existing probe unaffected. The now-stale `_note` on `config-verified-draft` is corrected (existence/property are emittable now → genuine `acceptable_variants`, not auto-fails; not forced into invariants, which would make the probe brittle to the LLM's coherent reading).
