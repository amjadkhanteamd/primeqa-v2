...
**Status:** Phase 0 (foundational design) — opening (D-111). Slice 1 (the `Interpretation` model + the deterministic interpreter) is the first build.

**Last substantive update:** 2026-05-27 (substrate opened)
...
- **Verdict (semantic):** a structured classification of what the outcome *means* for the requirement (e.g. `prohibition_enforced`, `prohibition_not_enforced`, `rejected_unasserted_reason`, `not_evaluated`, `asserted_metadata_present` / `…_absent`).
...
**Phase 0 (foundational design) — opening (2026-05-27).** The boundary (S4 captures truth, S6 interprets) and the deterministic-first commitment are locked (D-111). Next: slice 1 — the `Interpretation` model + the deterministic interpreter over both verticals' `RunEvidence`.
