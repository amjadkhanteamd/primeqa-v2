...
**Status:** Realized through Phase 5 (program), plus the **Verified / evaluation-strategy arc**. The deterministic interpreter covers the **full realized S4 execution surface** — inspection (existence + property) + the data-recipe behavioral negatives (1-step create-rejected AND the D-203 2-step update/delete-rejected, graded against the rejection-bearing mutation step) + the positive value-claim — persists eagerly in the run-path (D-111.2), and is consumable: cross-run clustering (D-116), LLM phrasing (D-117), and an in-substrate **read API + phrasing live-fire** (D-137). The **`Verified` evaluation-semantics** (D-272, evaluation-semantics-v1) and the **`errored` → indeterminate/permanent split** (D-273) are realized on the live interpret path. The **run-all / bva evaluation-strategy pipeline** — strategy arms `single`+`bva` (`strategy.py`, D-274/D-279), the decision-engine `Verified` seam + bva branch (D-280/D-283), the batch manifest + completeness reader (D-281/D-282, `batch_reader.py`), the async companion (D-284), the `strategy_kind` column on `test_claims` (D-285), the `_assemble` wiring (D-287), and the `EmissionBundle.strategy_kind` slot (D-288) — is **BUILT but DORMANT and INTENTIONALLY SUSPENDED at the architecture boundary (D-289)**: nothing authors `strategy_kind='bva'`, every claim routes `single`, and the arc is decision-neutral (byte-identical live behavior, zero Salesforce cost).
...
**Realized through Phase 5 (2026-06-03), plus the Verified / evaluation-strategy arc (2026-06-27).** The boundary (S4 captures truth, S6 interprets) and the deterministic-first commitment are locked (D-111). The build arc (see `EVOLUTION.md`): the deterministic interpreter + `Interpretation` model (D-111) → deeper VR-cause attribution through S1 (D-111.1 / D-114) → eager run-path persistence (D-111.2) → cross-run clustering (D-116) → LLM phrasing (D-117) → **full-surface verdicts** (D-136: the positive value-claim fix + property value verdicts, so the interpreter is correct + precise over everything S4 executes today) → **the in-substrate consumer** (D-137: a pure read API + the clustering reads re-exported + the phrasing live-fire, so S6 is no longer write-only). **Verified / evaluation-strategy arc:** the **`Verified` evaluation-semantics** (D-272, evaluation-semantics-v1) and the **`errored` → indeterminate/permanent split** (D-273) are realized on the live interpret path and frame the read-layer (ADR-001, D-270; Coverage spec, D-271). The **run-all / bva evaluation-strategy pipeline** is BUILT but DORMANT: the strategy arms `single`+`bva` (`strategy.py`, D-274/D-279), the decision-engine `Verified` seam + bva branch (`substrate_decision._claim_verified`, D-280/D-283), the batch manifest + completeness reader (D-281/D-282, `batch_reader.py`), the async companion (D-284), the `strategy_kind` column on `test_claims` (D-285), the `_assemble` wiring (D-287), and the `EmissionBundle.strategy_kind` slot (D-288). It is **INTENTIONALLY SUSPENDED at the architecture boundary (D-289)** — nothing authors `strategy_kind='bva'`, every claim routes `single`, the arc is decision-neutral (byte-identical live behavior, zero Salesforce cost); resumption is gated on a claim/evaluation-strategy modelling decision (4f.2a), not implementation. **Deferred:** the user-facing UI/dashboard over substrate runs + a standing production consumer (Phase-7 cutover); the dormant verticals' verdicts (ui/event/callout — no executor emits their evidence); cause attribution for positive/property failures; the reviewer edit/version lifecycle (S6-Q-005). See `DEFERRED_ITEMS.md`.
