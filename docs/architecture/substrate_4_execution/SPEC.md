...
**Status:** First vertical realized — **metadata-inspection**, built + merged + live-proven against the sandbox (bridge → executor → result store → finalize → run path). F1 / F2 / F3 realized; data verticals since: the 1-step create-rejected negative (D-110.2), the positive create-and-verify (D-115), provisioning + cleanup (F6, D-196), the enqueue loop (D-197), and the **2-step update/delete-rejected negative** (D-203 — setup create through the F6 machinery → rejected mutation → 4-way grading → teardown always), and **N-create positive chains with cross-step references** (D-205). Prohibition grading now recognizes a **permission/FLS rejection** (HTTP 403 + `INSUFFICIENT_ACCESS*`) as the org enforcing the prohibition by **access control** → graded `passed`, closing the hole where FLS/sharing negatives mis-graded `errored` (D-290 / D-290.1; engine-capability — unexercised on today's corpus). All three **automated execution triggers** are realized: on-approval enqueue (D-199), the CI release gate (D-198.3), and scheduled regression (D-214). See the footer Status block, `DEFERRED_ITEMS.md`, and `EVOLUTION.md`.
...
**Permission/FLS prohibition grading (2026-06-27; D-290 / D-290.1).** The `data-recipe` prohibition eval now treats a **permission/FLS rejection** (HTTP 403 + an `INSUFFICIENT_ACCESS*` code) as the org **enforcing the prohibition by access control** → graded `passed`, closing the hole where FLS/sharing negatives mis-graded `errored`. Two mirror grading sites in `execution_engine/data_executor.py` (`_run_create`, `_run_mutation_attempt`): the permission branch sits **after** the unchanged HTTP-400 VR business-rejection branch and routes through the **same** 4-way grade (passed iff `_matches`, else failed; succeeded-when-prohibited → failed + delete; couldn't-attempt → errored). D-290.1 hardening (from the pre-push adversarial review, which caught a real false-`passed`): the dual-meaning code `INVALID_FIELD_FOR_INSERT_UPDATE` is **excluded** from reclassification (`_AMBIGUOUS_PERMISSION_CODES`), and the branch is gated on **exactly HTTP 403** so a transient (5xx/429) or auth (401) response keeps its re-runnable `errored`/`indeterminate` signal (D-273). Code-only — no S2/`RejectionExpectation` change, no schema, no migration; the HTTP-400 VR path is byte-identical. **Unexercised on today's corpus** — S3 does not author permission-prohibition recipes yet, so no live verdict flips today (engine-capability).

**Automated execution triggers — all three realized.** On-approval enqueue (**D-199**), the CI release gate (`POST /api/webhooks/ci-trigger`, **D-198.3**), and scheduled regression (Build 6, the `s4_run_schedules` table + the scheduler's `s4_schedule_tick`, **D-214**) are all built — the manual queue endpoint is no longer the only path to a run.
...
- **F4 — recipe-kind scope: partially realized.** `metadata-recipe` (inspection) **and** `data-recipe` (the behavioral-negative create-rejected, D-110.1 → D-110.3) are built. The `data-recipe` prohibition grading now also recognizes a **permission/FLS rejection** (HTTP 403 + an `INSUFFICIENT_ACCESS*` code) as the org enforcing the prohibition by **access control** → `passed` (D-290; D-290.1 excludes the dual-meaning `INVALID_FIELD_FOR_INSERT_UPDATE` and gates on exactly-403 so transient/auth responses keep their re-runnable `errored`/`indeterminate` signal) — previously an FLS/sharing 403 negative mis-graded `errored`; engine-capability, unexercised on today's corpus (S3 does not author permission-prohibition recipes yet). Deferred: **positive** data-recipes (create→read→assert), update/delete-rejected negatives (need a provisioned record), and `ui-recipe` / `event-subscription-recipe` / `callout-intercept-recipe`.
