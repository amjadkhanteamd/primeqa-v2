...
- **F6 — test-data provisioning: minimal-cleanup realized; full provisioning deferred.** The behavioral negative needs no provisioning (a rejected create makes nothing); the **targeted best-effort delete** of an unexpectedly-created record (N-5) is built. Full provisioning + a created-record tracking table land with positive CRUD; required master-detail/lookup → a parent record stays deferred (S3-A / F6).
