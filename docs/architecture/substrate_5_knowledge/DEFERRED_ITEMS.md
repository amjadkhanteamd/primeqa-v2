...
- **The S3-substrate generation forward-seam.** The dependency graph has S5 feeding S3, and `primeqa/generation/protocol.py` already carries an attribution stub (*"Domain packs invoked (the S5 knowledge channel), if any"*). But the substrate generation **emits semantic claims** (existence/property/capability/…), a different artifact from v1's test cases — and the existing knowledge (domain packs about Case-escalation test patterns; system rules like "formula fields are read-only") is **v1-test-case-calibrated**. Wiring the S5 port into the S3-substrate prompt therefore needs a **semantic-fit design first**: what knowledge does a *claim-emitter* consume, and in what prompt block? Deferred until that's defined (and until the v1-vs-substrate generation direction settles at the cutover). — **D-134** **→ RESOLVED (D-156, cutover Step 2): closed for the cutover — not wired.** The direction settled as "v1 generation stays as the product path (data-source REPLACE onto S1); S3 is parallel + downstream-only, **not** product-facing", and the semantic fit is ~zero (S5 is test-authoring-calibrated; S3's claim-emitter needs ontology signals already enforced by `governance_core`). A wire would be a **net-new** claim-ontology channel, gated on a future decision making S3 product-facing — which the cutover explicitly does not make.
