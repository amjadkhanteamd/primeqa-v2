...
| Artifact type | Detection coverage | Instrument notes |
|---|---|---|
| Validation rules | **52/52 active rules** (formula + active flag, all 8 shape families) | 22 retroactive events, 1:1 vs SetupAuditTrail |
| Picklist value sets | **377 picklist fields**: membership reliable for **352** (marks `inline` 56 + `inline_standard` 296); **17 `inline_truncated` + 8 `no_values` UNKNOWN-guarded** (D-399.1 — never silent); value activation for all ~7k captured values | 64 retroactive events; the D-403/D-414 repair = ONE `CAPTURE_GENERATION` event (in-data suppression); seq-184 platform-release values caught as real MEMBERSHIP |
| Flows | **activation + version-moved only** (35 flows) — content invisible by construction; the open-snapshot diff (D-427) stays the content instrument | 24 retroactive events; the F1 churn = `VERSION_MOVED 1→3`; the HL_Auto_Submit_Approval org fix retroactively pinned to seq 120 (2026-07-08) — the change D-428 recorded as unnoticed for three weeks |

**Exits under the amended criterion, stated per family:** all 8 **VR** shape
families — EXIT MET (DETECTION), per the §1.2 row and its qualification.
**AfterSave divergent-value** — MET under form (a) (attribution, D-429). All
other flow families — **no change**: detection cannot reach flow logic, so
they exit only under form (a), and their §2.2 standing (reachable /
BLOCKED / natural-specimen status) is unchanged. Picklists are a constraint
layer, not claim families — their detection coverage is recorded above but
no §3 exit applies.
...
| 2026-08-09 (detector extensions) | **D-437: the drift detector extends to picklist value sets and flow activation** (branch `phase-1-substrate-1-drift-detector-extensions`, merge pending GO). Retroactive acceptance: picklist 64 events — the D-403/D-414 capture repair collapses to ONE `CAPTURE_GENERATION` event (377 marks + 313 sets + 6,230 values; the suppression is in-data — the marks transition in the same rows at the same seq — not heuristic), seq-184 (08-07) platform-release picklist values caught as 2 real MEMBERSHIP events with 3 truncated-capture UNKNOWNs correctly quarantined; flow 24 events — the F1 window churn appears as `VERSION_MOVED 1→3` (content-unknown by construction) and the HL_Auto_Submit_Approval org fix is retroactively pinned to seq 120 / 2026-07-08 (the unnoticed change D-428 recorded). §2.1 gains the detection-coverage-per-type table + per-family exit statement. Detection coverage: VRs 52/52, picklist membership 352/377 fields (25 UNKNOWN-guarded), flows activation-only. |
