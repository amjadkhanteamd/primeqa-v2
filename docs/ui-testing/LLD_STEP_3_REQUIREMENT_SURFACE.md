...
**Status: BUILT (2026-09-09; design 27ee7fc, GO with all five forks on the leans and the inventory lifecycle flag recorded as fork 3's successor). Five corrections found at build are recorded in VERIFICATION_STEP_3.md §0 and folded below: the approve hook re-materialises against the set just approved (`rematerialise(claim_set_id=)`); the HTMX redirect carries no fragment; the console has a `session=` seam; the active-environment filter is keyed on `tenant_id` (None = unfiltered, the harness); NEEDS_HUMAN verdicts fold into "not determined" in the split (ledgered Low).** Branch `step-3-requirement-surface`
...
`_environments_with_evidence(session, test_ids, *, tenant_id=None)` gains the active filter at its one seam (applied when `tenant_id` is given — both production callers pass it; `None` keeps the unfiltered read for the tenant-only local harness, which carries no `public.environments`) — both callers (the engine's per-environment loop at `:904` and `release_scope_readiness` at `:1002`) enumerate through it, so the scope check and the decision's branches agree:
