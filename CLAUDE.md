...
- **Public release-status endpoint is token-gated** (migration 055): `GET /api/releases/:id/status` requires a per-release opaque poll token (`status_poll_token_hash`, sha256); no token → 401, wrong token → 404 (neither reveals whether the id exists). Mint, rotate and revoke are the release's OWNER (`releases.created_by`) or an admin, tenant-scoped — enforced in the routes since AUD-012 (triage round 2), and every act route's tier and owner rule is declared in `tests/unit/test_route_authority_table.py`.
...
- **Input validation**: every create/update SERVICE checks each value against its COLUMN before any row is built — `primeqa/shared/validation.py` (`columns(model, values)` reads the bounds from the SQLAlchemy model: `String(n)` length, integer range, boolean, date, JSON shape, NUL-free; a refusal is a `ValueError` naming the field → 400). Release, requirement, section, environment, connection and group services apply it (AUD-011, triage round 2); `feedback.capture_user_feedback` type-checks verdict; bulk endpoints coerce ids to positive ints. The boundary gate `tests/integration/test_boundary_gate.py` POSTs empty / 20k-character / wrong-type bodies to EVERY body-reading write route of the url_map and fails on any 500 — the database is never the validator.
