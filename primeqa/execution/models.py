...
class PipelineRun(Base):
    ...
    # Rich provenance for wizard-based runs (Q7): mixed Jira/suite/hand-picked
    # selections are stored here so "Rerun" can replay the exact payload.
    source_refs = Column(JSON, nullable=False, server_default="{}")
    # Rerun lineage (populated by R5 agent auto-fix loop)
    parent_run_id = Column(Integer, ForeignKey("pipeline_runs.id", ondelete="SET NULL"))
...
class RunStepResult(Base):
    ...
    soql_queries = Column(JSON)
    llm_prompt_sha = Column(String(64))
    llm_response_sha = Column(String(64))
