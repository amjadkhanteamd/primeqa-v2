...
def _keys_to_test_ids(session, keys) -> dict:
    ...
    (COVERAGE_LINK_KINDS membership, deduped per key). One link query for all
    keys — the corpus loop was one round trip per requirement."""
...
def get_landing_substrate_stats(tenant_id: int, environment_id=None) -> dict:
    ...
    and the evidence-derived pass_rate/flaky) to one connected org; ``None``
    keeps the tenant-wide view. Claim counts are always tenant-wide — claims
    are org-agnostic (authored once, run per org)."""
    from sqlalchemy.orm import Session
