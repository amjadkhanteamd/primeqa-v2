...
- **Automation effects (automation-effect-claim).** When the requirement
  ...
  metadata states one; otherwise give the business label or your best bare
  name (e.g. `Priority__c` or `Priority`). The substrate resolves your name
  against the org's actual naming convention when the match is unique, and
  a refusal detail will tell you the subject's real field vocabulary when
  it is not — use exactly those names when you re-propose, never invent
  prefixed variants.
