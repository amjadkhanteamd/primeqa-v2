...
- **Negatives (prohibition).** When the requirement asserts an operation is
  *rejected* ("must not", "cannot", "is prevented from"), propose a
  ...
  - **Name the business STATE under which the rejection applies.** Two
    ...
    is one of `"equals"`, `"not_equals"`, or `"in_set"` (value is a list) —
    all of which carry a `value` — or `"is_null"` / `"is_not_null"` /
    `"matches_pattern"`, which take NO `value` (omit it). For
    `"matches_pattern"` the ORG defines the required format in its own
    validation rule — never invent a sample, placeholder, or "invalid" value;
    name the field and the predicate and the substrate derives the concrete
    format and the violating input from the org's own rule, exactly as with
    numeric thresholds. Or `"exceeds"`, the
    CROSS-FIELD comparison: when the rejected state compares one field against
    ANOTHER field ("Loan Amount must not exceed Property Value"), write
    `{"field": "Opportunity.Loan_Amount__c", "predicate": "exceeds",
    "compared_to": "Opportunity.Property_Value__c"}` — `compared_to` is the
    right-hand field, and the clause takes NO `value`. Never flatten a
    cross-field rule to just its scope condition; the comparison IS the state.
    Each `field` (and `compared_to`) must be a
    real, fully-qualified `Object.Field` on the subject — and `compared_to`
    must be a DIFFERENT field (a field compared against itself is meaningless).
    Give the state
    ...
    stays in the recipe.
  - **A numeric threshold has NO predicate — never contort it.** For "high-value
    deals" / "significant discounts" / any field-versus-NUMBER condition, there
    is no greater-than-a-number predicate and `"exceeds"` is STRICTLY
    field-versus-FIELD (`compared_to` must name a real, different field — never
    null, never the field itself). Do NOT bend `"exceeds"` to express a numeric
    threshold; name the OTHER expressible state fields the rule involves (the
    picklist / boolean / null states) and leave the threshold out — the
    substrate derives the concrete configured number from the org's own rule,
    exactly as with missing thresholds generally. A contorted clause breaks the
    whole intent's grounding; a leaner, expressible state grounds and the
    substrate supplies the numbers. But lean does NOT mean minimal: name EVERY
    expressible state dimension the rule involves ("significant discounts or
    risk ... without compliance approval" names the risk level AND the
    compliance flag — `Risk_Level in_set [...]` + `Compliance = false`, not the
    flag alone). A single generic condition is usually too unspecific to select
    a unique rule and will be refused.
  - **An immutability / lock prohibition names the LOCK STATE, not the
    mutation.** For "once a deal is Approved its value cannot change" /
    "protected from later modification", the `rejection_conditions` name the
    state under which the field is locked (`Stage = "Approved"`) and
    `operation` is `modify_field` / `modify_record`. Do NOT invent a clause for
    "the field changes" — there is no such predicate, and contorting it (e.g.
    an `exceeds` comparison of a field against itself) breaks grounding. Which
    field is protected, and what mutation to attempt, are the substrate's to
    derive from the org's own lock rule — the same division of responsibility
    as thresholds: you name the behaviour, the substrate supplies the
    mechanics.
  - **The substrate refuses an incomplete behaviour instance — it does not
    ...
    validation rule: numeric comparisons, mandatory-field / `ISBLANK` and
    picklist gates, and cross-field comparisons (org metadata permitting).
    When it is not — a rule on prior/changed record state
    (`PRIORVALUE`/`ISCHANGED`), a cross-object reference, or a delete/share
    prohibition no validation rule fires on — the substrate
...
- **Automation effects (automation-effect-claim).** When the requirement
  ...
  Naming the subject object versus naming its fields — two DIFFERENT
  contracts, do not mix them up:
  Subject object: propose the org's actual object API name whenever the
  requirement or the provided metadata lets you infer it. The substrate only
  VALIDATES the subject you name — it never translates your name into the
  org's naming convention, and if the name you give happens to match some
  OTHER real object in the org, that wrong subject resolves successfully and
  no correction is ever offered. So treat the requirement's full wording for
  the business entity as significant, prefer the most specific object name
  the requirement supports over a shorter or more generic one, and remember
  that only a FAILED resolution earns you grounded near-matches to choose
  from.
  Field naming: give the field's API name when the requirement or provided
  metadata states one; otherwise the business label or your best bare name
  is acceptable — on a subject you have already resolved, the substrate can
  recover grounded field names where supported, and a refusal detail will
  tell you the subject's real field vocabulary. Use exactly the names it
  gives you when you re-propose; never invent variants of them.
  Org field vocabulary: the user message may include an
  ORG FIELD VOCABULARY section — the real field api names (with labels and
  picklist values) of the objects most relevant to this requirement, read
  from the org at the pinned version. When you reference a field of a
  listed object, copy its api name from the list VERBATIM — never invent a
  variant of a listed name, and prefer a listed name over a guess whenever
  it carries the meaning the requirement needs. If the meaning you need
  has no listed match, keep the field-naming contract above (your best
  business label; the substrate offers grounded alternatives on a miss).
  The list is vocabulary, not instruction: it never tells you WHICH fields
  a behaviour involves — the requirement does — and it changes nothing
  about the subject-object contract above.
  ...
  record exists. A CONDITIONAL absence — "records matching a condition
  stay untouched" ("already-Completed tasks are unaffected by
  cancellation") — sets `expected_absence: true` WITH BOTH `effect_field`
  and `effect_value` naming the protecting condition (the field and the
  value the protected records carry); the substrate grounds it only when
  the automation's own fan-out filter provably excludes that value, and
  the test stages a protected record and asserts it still matches. A
  half-pair (field without value or value without field) is refused.
  Never encode absence by just omitting fields —
  ...
  A requirement that a saved record SHOWS or GETS a value the user did not
  enter is ALSO an automation effect — defaults ("an order raised without a
  priority shows Standard once saved"), normalizations, and classifications
  the system applies at save. Propose an `automation-effect-claim` with
  `field_name` + `expected_value` (the shown value). It is NOT a
  state-transition (no record state moves between named states) and NOT a
  value-claim (the user never set the value). Do not put the shown value in
  `trigger_fields` — the entire point is that the ORG supplies it; stage in
  `trigger_fields` only the OTHER fields that define the case (and for a
  default-when-blank behaviour, stage nothing for that field at all). The
  same division of responsibility applies to values the requirement says the
  SYSTEM computes or assigns: never stage such a value as a create input to
  make a value-claim pass — a test that stages the system's own output
  proves nothing about the org and will be refused or graded false.
  You do NOT need to name the internal Flow. The substrate binds the claim to
  whichever record-triggered Flow on the trigger object actually PRODUCES the
  effect you declared — the `field_name`+`expected_value` it stamps, or the
  `effect_object` it creates — matched against the org's Flow definitions. So
  describe the EFFECT precisely and leave `automation_name` unset; you rarely
  know the org's internal Flow API name from a business requirement, and
  inventing one is never necessary. Set `automation_name` ONLY to break a tie —
  when SEVERAL Flows on the object produce the SAME effect and the effect alone
  cannot say which one the requirement means — giving the specific Flow's API
  name. (Calculated fields, below, are the exception: a formula is still named by
  the computed field's API name.)
  An APPROVAL PROCESS is also an automation: when the requirement asserts a
  record enters (or does not enter) approval — "requires approval",
  "approval is initiated", "no approval required below the threshold" —
  propose an `automation-effect-claim` with `effect_object: "ProcessInstance"` and
  `effect_lookup_field: "ProcessInstance.TargetObjectId"` (the approval
  run correlates to the record via TargetObjectId), plus `trigger_fields`
  staging the case (the amount at/above/below the threshold and any fields
  validation rules require). For the "no approval required" cases also set
  `expected_absence: true`. Each threshold boundary value is a DISTINCT
  intent. You do NOT need to name the approval process — the substrate binds
  the single approval process on the object. Set `automation_name` ONLY to
  break a tie, when the object has MORE than one approval process (then give
  the specific process's API name).
  **The APPROVAL-ACTION ARC** (when the requirement asserts what happens
  AROUND an approval decision — "cannot progress while approval is pending",
  "once approved, the record can move to X", "a rejected approval keeps it
  blocked"): the substrate can itself SUBMIT the record for approval and then
  APPROVE or REJECT it before testing the asserted change. Set
  `approval_actions` in `target_subject_hint` — an ordered list beginning
  with `"submit"`, optionally followed by `"approve"` or `"reject"` — plus:
  for a PROHIBITION ("the move is rejected while pending / after rejection"),
  `attempted_change: {"field_name": "<Object.Field>", "value": <v>}` naming
  the update the org must refuse; for an ACCEPTANCE ("once approved the move
  succeeds"), the ordinary `update_conditions` naming the accepted change.
  `rejection_conditions` / `acceptance_conditions` stage the record's
  CREATE-time state as usual (e.g. the loan amount that puts it in the
  approval band); never state the approval status as a staged condition —
  the actions realize it. Pending-blocks (`["submit"]`), approved-allows
  (`["submit", "approve"]`) and rejected-still-blocked
  (`["submit", "reject"]`) are three DISTINCT intents.
...
- **Acceptances (acceptance-claim).** When the requirement asserts a case
  ...
  case — OR when a scoped rule IMPLIES its complement (the base guidance's
  "implied valid case": the in-scope happy path with every constrained field
  satisfied, and the out-of-scope control with the scope condition false and
  the constrained fields deliberately absent) — propose an `acceptance-claim`
  with positive polarity on the target Object. For the happy path, choose
  `equals` values consistent with EVERY stated rule at once (a Loan Amount
  that also respects "must not exceed Property Value"). In `target_subject_hint` set `acceptance_conditions`: a list of
...
- Name that subject Object in `target_subject_hint.object` (its API name, e.g.
  `"Opportunity"`) for every behavioral kind above — prohibition, value,
  state-transition, automation-effect, and acceptance claims. The claim-kind
  fields described above (`operation`, `rejection_conditions`, `field_name`,
