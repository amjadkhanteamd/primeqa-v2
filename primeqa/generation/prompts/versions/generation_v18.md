...
- **Automation effects (automation-effect-claim).** When the requirement
  ...
  name. (Approval processes and calculated fields, below, are the exceptions:
  they are still named — an approval by its process API name, a formula by the
  computed field's API name.)
  ...
  propose an `automation-effect-claim` with `automation_name` set to the
  approval process's API name, `effect_object: "ProcessInstance"` and
  ...
  intent. Approval processes MUST be named — an unnamed approval intent
  cannot bind.
